...
z_score <- qnorm((1 + confidence_level) / 2)  # 计算z值
sigma0=1
# 循环模拟数据和估计参数
for (i in 1:num_simulations) {
  ...
  estimated_params <- coef(fit_nls)  # 获取估计的参数值
  print(paste("Iteration:", i, "Estimated Params:", estimated_params))
  param_se <- summary(fit_nls)$coefficients[, "Std. Error"]  # 获取标准误差
...
for (i in 1:num_simulations) {
  ...
  # Step 3: 使用 tryCatch 捕获 nlsLM 中的错误
  library(minpack.lm)
  fit_nls <- tryCatch(
    {
      nlsLM(
