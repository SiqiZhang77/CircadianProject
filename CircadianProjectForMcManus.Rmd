...
```
The code uses the estimated parameters for the \textit{time} $<$ 100 and \textit{time} $\geq$ 100 periods to generate predicted values using the \texttt{asym\_model\_function}. The original data points (in black) for both periods are overlaid with the model's predicted curves (in blue for \textit{early} and red for \textit{late}). The objective is to visually assess how well the model's predictions (based on estimated parameters) match the original data for both periods.
...
hist(variances, main = "Histogram of Variances",
     ...
     col = "lightblue",
     border = "black")
# Initial parameter estimates 111
